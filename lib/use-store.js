...
export const SERVICE_TYPES      = ["Placement", "New Placement"];
...
const useDashboardStore = create((set, get) => ({
  ...
  loading:      true,
  dbError:      null,
  // Removed currentPage as we are now using Next.js routing
  toast:        null,
  notifications: [],   // loaded from DB on boot via loadFromDB

  /* NOC target — set when user clicks "Generate NOC" in Notifications page */
  nocTargetCandidate: null,  // candidate name to pre-fill in CandidateHistory
  nocTargetNotifId:   null,  // notification id so we can mark noc=true after NOC is generated
  ...
  navigate: (page) => {
    // Left empty or we can remove entirely if no other components call it
    // Wait, components might call navigate() somewhere else! Let's check.
  },
  ...
  addNotification: async (notification) => {
    ...
    // Optimistic update
    set((state) => ({ notifications: [entry, ...state.notifications] }));
    try {
      ...
      if (res.ok) {
        const { notification: saved } = await res.json();
        if (saved) {
          set((state) => ({
            notifications: state.notifications.map(n => String(n.id) === String(id) ? saved : n),
          }));
        }
      }
    } catch (e) {
      console.warn("addNotification network error:", e);
    }
    return id;
  },
  markNotificationRead: async (id) => {
    ...
    try {
      ...
      console.warn("markNotificationRead network error:", e);
    }
  },
  deleteSpecificNotification: async (id) => {
    // Optimistic update
    set((state) => ({
    ...
    try {
      ...
      console.warn("deleteSpecificNotification network error:", e);
    }
  },
  markNotificationNoc: async (id) => {
    // Optimistic update
    set((state) => ({
      notifications: state.notifications.map((n) => String(n.id) === String(id) ? { ...n, noc: true } : n),
    }));
    try {
      await fetch(`/api/notifications/${encodeURIComponent(id)}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ noc: true }),
      });
    } catch (e) {
      console.warn("markNotificationNoc network error:", e);
    }
  },
  setNocTarget: (candidate, notifId) => set({ nocTargetCandidate: candidate, nocTargetNotifId: notifId }),
  clearNocTarget: () => set({ nocTargetCandidate: null, nocTargetNotifId: null }),
  deleteAllNotifications: async () => {
    ...
    try {
      ...
      console.warn("deleteAllNotifications network error:", e);
    }
  },
  ...
  loadFromDB: async () => {
    ...
    try {
      const [entriesRes, expensesRes, notifRes] = await Promise.all([
      ...
      set({ entries: entries || [], expenses: expenses || [], notifications: notifications || [], loading: false });
    } catch (e) {
  ...
  updateStatus: async (id, status) => {
    ...
    // Determine if this is a terminal (Laid Off / Default) sweep
    const isLaidOffSweep = LAIDOFF_STATUSES.includes(status);
    const isDefaultSweep = status === "Default";
    const isTerminalSweep = isLaidOffSweep || isDefaultSweep;
    const terminalScope = isLaidOffSweep ? "laidoff" : isDefaultSweep ? "defaulter" : null;

    // Compute the cascade using the pure planner. For terminal sweeps, the
    // clicked row keeps sheetScope="payment" (red anchor); other Pending rows
    // for the same candidate are swept to the terminal sheet.
    ...
    // Build the clone of the original clicked row (goes to the terminal tab)
    // We add it to the local state immediately for instant UI feedback.
    const now = new Date();
    const currentMonthName = MONTH_NAMES[now.getMonth()];
    const prevMonthName = MONTH_NAMES[(now.getMonth() - 1 + 12) % 12];
    const entryMonth = (entry?.month || "").trim().toLowerCase();
    const isMonthMatch = entryMonth === currentMonthName.toLowerCase() || entryMonth === prevMonthName.toLowerCase();

    const cloneEntry = (isTerminalSweep && isMonthMatch)
      ? {
          ...entry,
          id: String(Date.now()) + "-clone",
          status,
          paid: 0,
          due: parseFloat(entry.amount) || 0,
          sheetScope: terminalScope,
        }
      : null;

    const toSend = cascaded.filter(e => changedIds.has(String(e.id)));
    // Optimistically update state — include the clone if this is a terminal sweep
    set({ entries: cloneEntry ? [...cascaded, cloneEntry] : cascaded });

    try {
      // Write changed rows (including swept rows) to DB
      const res = await fetch("/api/entries", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      // Persist the clone to DB separately (it's a new row)
      if (cloneEntry) {
        const cloneRes = await fetch("/api/entries", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(cloneEntry),
        });
        if (cloneRes.ok) {
          const { entry: savedClone } = await cloneRes.json().catch(() => ({}));
          if (savedClone) {
            set(s => ({
              entries: s.entries.map(e =>
                String(e.id) === String(cloneEntry.id) ? savedClone : e
              ),
            }));
          }
        }
      ...
      // Check if this "Received" update pushed the candidate to 100% payment completion
      if (status === "Received") {
        const candidateName = (entry.candidate || "").trim();
        if (candidateName) {
          const currentEntries = get().entries;
          const candidateEntries = currentEntries.filter(
            e => (e.candidate || "").trim().toLowerCase() === candidateName.toLowerCase()
          );
          ...
          if (pct === 100) {
            const alreadyNotified = get().notifications.some(
              n => n.type === "payment-complete" && n.candidate === candidateName
            );
            if (!alreadyNotified) {
              const message = `${candidateName} has fully paid the amount. Please update the NOC`;
              get().addNotification({ candidate: candidateName, message, type: "payment-complete" });
              get().showToast(`🎉 ${message}`, 5000);
              return; // toast already shown, skip the generic one below
            }
          }
        }
      }
      ...
      if (LAIDOFF_STATUSES.includes(status)) {
        get().showToast(
          cascadeCount > 1
            ...
            : `🔴 Laid Off — entry flagged red; clone moved to Laid Off sheet`
        );
      } else if (status === "Default") {
        get().showToast(
          cascadeCount > 1
            ? `🔴 Default — ${cascadeCount - 1} other pending entr${cascadeCount - 1 === 1 ? "y" : "ies"} also moved to Defaulter sheet`
            : `🔴 Default — entry flagged red; clone moved to Defaulter sheet`
        );
