...
/* ── Status routing ───────────────────────────────────── */
export const ROUTED_STATUSES    = ["Laid Off", "Offer Revoke", "No Offer", "Resigned", "Default"];
export const LAIDOFF_STATUSES   = ["Laid Off", "Offer Revoke", "No Offer", "Resigned"];
export const ALL_STATUSES       = ["Paid", "Pending", "Laid Off", "Offer Revoke", "No Offer", "Resigned", "Default"];
...
export function computePaidDue(entry) {
  ...
  if (entry.status === "Paid")    return { ...entry, paid: amount, due: 0 };
...
const useDashboardStore = create((set, get) => ({
  ...
  /* ── Derived getters ── */
  getActive:    ()  => get().entries.filter(e => !ROUTED_STATUSES.includes(e.status)),
  getLaidOff:   ()  => get().entries.filter(e => LAIDOFF_STATUSES.includes(e.status)),
  getDefaulters:()  => get().entries.filter(e => e.status === "Default"),
  ...
  updateStatus: async (id, status) => {
    ...
    for (const e of prev) {
      ...
      if (e.status === "Paid") continue;            // never override Paid rows
  ...
  /* ── Bulk delete (atomic: one round trip, all-or-nothing rollback) ── */
  ...
  /* ── Load NOC/admin settings ── */
  loadNOCSettings: async () => {
    if (get().nocSettings) return get().nocSettings;
    try {
