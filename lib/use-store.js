...
const useDashboardStore = create((set, get) => ({
  ...
  updateStatus: async (id, status) => {
    ...
    const TERMINAL = ROUTED_STATUSES;
    const CANDIDATE_WIDE = new Set(["Received", "Pending", "Laid Off", "Default"]);
    const entry = get().entries.find(e => String(e.id) === String(id));
    if (!entry) return;

    const prev = get().entries;
    const candidate = (entry.candidate || "").trim().toLowerCase();
    const affectAllSameCandidate = !!candidate && CANDIDATE_WIDE.has(status);

    if (affectAllSameCandidate) {
      const affected = prev.filter(e => (e.candidate || "").trim().toLowerCase() === candidate);
      ...
      const cascaded = prev.map(e => {
        ...
        return {
          ...
          sheetScope: scopeForStatusUpdate(e.sheetScope, status),
        };
      });
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.error("updateStatus candidate cascade failed:", err);
          return;
        }
        const { entries: saved } = await res.json();
        if (Array.isArray(saved) && saved.length) {
        ...
        get().showToast(`✓ Status → ${status} for ${affected.length} candidate entr${affected.length === 1 ? "y" : "ies"}`);
      } catch (e) {
        set({ entries: prev });
        get().showToast("✕ Status update failed — network error");
        console.error("updateStatus candidate cascade network error:", e);
      }
      return;
    }
    ...
    if (!TERMINAL.includes(status)) {
      ...
      const patch  = {
        ...
        paid: status === "Received" ? amount : 0,
        due:  status === "Received" ? 0      : amount,
        sheetScope: scopeForStatusUpdate(entry.sheetScope, status),
      };
    ...
    const cascaded = prev.map(e => {
      ...
      return { ...e, status, paid: 0, due: amt, sheetScope: scopeForStatusUpdate(e.sheetScope, status) };
    });
