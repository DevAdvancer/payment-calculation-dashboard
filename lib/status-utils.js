...
export function routeBucketForEntry(entry) {
  /* `sheetScope` is the authoritative routing field — it tells the UI
     ...
     label (used for filtering within a page). When a row is swept
     onto the Laid Off page by the cascade, its `status` stays
     "Pending" but its `sheetScope` becomes "laidoff"; that row MUST
     surface on the Laid Off page, not the Payment page. */
  ...
  /* No explicit scope (or "payment") — fall back to status for
     legacy rows that pre-date the sheetScope field. */
