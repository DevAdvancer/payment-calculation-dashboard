import { z } from "zod";
import { normalizeCompanyName } from "./company-utils.js";
import { ALL_STATUSES as STATUS_VALUES } from "./status-utils.js";
...
export const EntrySchema = z.object({
  ...
  company: z.string().nullable().optional().transform((v) => (v ? normalizeCompanyName(v) : v)),
  poDate: dateField,
  dueDate: dateField,
  ...
  currency: z
    ...
    .nullable()
    .optional()
    .refine((v) => !v || ["USD", "GBP", "INR"].includes(v), {
      message: "Currency must be USD, GBP, or INR",
    }),
  sheetScope: z
    .string()
    .nullable()
    .optional()
    .refine((v) => !v || ["po-details", "payment", "laidoff", "defaulter"].includes(v), {
      message: "Sheet scope must be po-details, payment, laidoff, or defaulter",
    }),
});
...
export function formatValidationErrors(errorOrIssues) {
  const issues = Array.isArray(errorOrIssues)
    ? errorOrIssues
    : errorOrIssues?.issues || errorOrIssues?.errors || [];
