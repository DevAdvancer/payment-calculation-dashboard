...
/* ── Helper: apply a "sweep onto terminal sheet" cascade.
   ...
   Rows BEFORE the clicked row (Aug, Sep, etc.) are left untouched.
...
function sweepCandidate(entries, clickedEntry, sameCandidate, terminalStatus, terminalScope) {
  ...
  const clickedMonth = (clickedEntry.month || "").trim().toLowerCase();
  const isMonthMatch = clickedMonth === currentMonthName.toLowerCase() || clickedMonth === prevMonthName.toLowerCase();
  ...
  return entries.map(e => {
    ...
    if (id === clickedId) {
      ...
      // if its month matches the current calendar month (anchor row).
      // Otherwise, it moves directly to the terminal sheet.
      ...
      return {
        ...
        paid: 0,
        due: amt,
        sheetScope: isMonthMatch ? "payment" : terminalScope,
      };
    }
    if (sweepIds.has(id)) {
