...
export async function createNotification(notif) {
  await ensureTables();
  const rows = await sql`
    INSERT INTO notifications (id, candidate, message, type, read)
    VALUES (
      ${String(notif.id)},
  ...
  const r = rows[0];
  return { id: r.id, candidate: r.candidate, message: r.message, type: r.type, read: r.read, createdAt: r.created_at };
}
