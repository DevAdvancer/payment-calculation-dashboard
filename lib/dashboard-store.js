...
export async function ensureTables() {
  ...
  await sql`
    ...
    VALUES ('admin@company.com', 'Admin User', ${hash}, 'admin', 'active')
    ON CONFLICT (email) DO NOTHING
  `;

  await sql`
    INSERT INTO users (email, name, password_hash, role, status)
    VALUES ('user@company.com', 'Regular User', ${hash}, 'user', 'active')
    ON CONFLICT (email) DO NOTHING
  `;
...
export async function createUser(user) {
  ...
  const rows = await sql`
    INSERT INTO users (email, name, password_hash, role, status)
    VALUES (${user.email}, ${user.name}, ${hash}, ${user.role || 'user'}, ${user.status || 'active'})
